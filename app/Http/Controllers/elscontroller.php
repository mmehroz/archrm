...
class elscontroller extends Controller
{
				...
				if($task){
				  ...
				  // 'storeid' => $task->store_uid,
					...
					if(session()->get("email")){
						...
						if($empEmailMobile[1] == "arcinventador.com"){
							if($task){
								...
								if ($task->elsemployees_password == "Arc_tech20222") {
	...
	public function empsave(Request $request){
		...
		if($empBatchID == "" && $empEmail == "") {
			...
			$post->elsemployees_password = "Arc_tech2022" ;
			...
			if($created){
				...
				// ->cc('hr@arcinventador.com')
				// ->bcc(session()->get("email"))
				// ->bcc('muhammad.mehroz@arcinventador.com')
...
//End Code
	...
	public function emprequest(){
			if(session()->get("email")){
				...
				}elseif(session()->get("email") == "salman.khairi@arcinventador.com"){
	...
	public function viewemprequest(){
			if(session()->get("email")){
				...
				if(session()->get("role") <= 2 && session()->get("email") != "salman.khairi@arcinventador.com"){
    ...
    public function decline($id){
        $empleavetable = elsleaverequest::on('mysql')->findOrFail($id);
		  if(session()->get("role") == 3 && session()->get("email") == "salman.khairi@arcinventador.com"){
		  ...
		  $empleavetable->elsleaverequests_changeby = session()->get("name");
		  $updatedleave = $empleavetable->save();
		  }elseif(session()->get("role") == 3 && session()->get("email") != "salman.khairi@arcinventador.com"){
		  $empleavetable->elsleaverequests_status = "Decline";
	...
	public function approve($id,$comment){
		...
		if($id){
			...
			if($task1->elsleaverequests_totalleavedays >= "3" && $task1->elsleaverequests_status == "Pending" ){
				if(session()->get("email") == "salman.khairi@arcinventador.com"){
					$empleavetable = elsleaverequest::on('mysql')->findOrFail($id);
		...
		if($updatedleave){
				...
				// ],
				// function ($message) use ($requester) {
				// $message->to('salman.khairi@arcinventador.com')
				// ->bcc('muhammad.mehroz@arcinventador.com')
				// ->cc(session()->get("email"))
				// ->cc($requester)
		...
		if($updatedleave){
				...
				// ],
				// function ($message) use ($requester) {
				// $message->to('hr@arcinventador.com')
				// ->bcc('muhammad.mehroz@arcinventador.com')
				// ->cc(session()->get("email"))
				// ->cc($requester)
				// ->subject('Employee Leave Approved By: ' . session()->get("name"));
...
// dd($udate);
    ...
    public function forget_submit(Request $get){
      ...
      if($cmd){
        $toemail = $get->email;
              Mail::send('email.forget_password', ['name' => 'Arc-Inventador',
                    'email' => 'hr@arcinventador.com','verify_token' => $verify_token,],
